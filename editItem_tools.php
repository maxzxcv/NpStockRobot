...
if (!mysqli_query($con, $logQuery)) {
    ...
    die("Error logging action: " . mysqli_error($con));
        ...
        echo "<script>alert('แก้ไขข้อมูลสำเร็จ'); window.location.href = 'stock_tools.php';</script>";
    } else {
...
<body>
    ...
    <select class="form-control" id="<?= $key ?>" name="<?= $key ?>">
        ...
        <option value="not_active" <?= ($value === 'not_active') ? 'selected' : '' ?>>เสีย ❌</option>
            ...
            <?php elseif (in_array($key, ['id', 'ItemName','NumberItem' , 'image'])) : ?>
                <!-- Read-only Field -->
    ...
    <a href="stock_tools.php" class="btn btn-secondary">ยกเลิก</a>
    <a href="#" class="btn btn-danger" onclick="confirmDelete(<?= $id ?>, 'Stock_Tools')">ลบทิ้ง</a>
</form>
    </div>
</body>
                ...
                .then(data => {
                    if (data.includes("ลบข้อมูลสำเร็จ")) {
                        Swal.fire({
                            ...
                            window.location.href = 'stock_tools.php';
                        });
                    } else {
