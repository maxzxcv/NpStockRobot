...
if (!mysqli_query($con, $logQuery)) {
    ...
    die("Error logging action: " . mysqli_error($con));
        ...
        echo "<script>window.location.href = 'outofstock.php';</script>";
...
$column_labels = [
    ...
    'date' => 'วันที่นำออก',
    'ProductName' => 'ชื่อ',
    'itemoutstock' => 'จำนวน น้อยกว่าหรือเท่ากับเท่า จึงแจ้งเตือนว่าของใกล้หมด',
    'image' => 'รูปภาพ'
...
<body>
    ...
    <select class="form-control" id="<?= $key ?>" name="<?= $key ?>">
        ...
        <optgroup label="สถานะผิดปกติ">
            ...
            <?php elseif (in_array($key, ['id', 'ItemName', 'whereItem','image','NumberItem'])) : ?>
                <!-- Read-only Field -->
    ...
    <a href="outofstock.php" class="btn btn-secondary">ยกเลิก</a>
</form>
    </div>
</body>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(event) {
      ...
      }, 500); // ใส่เวลารอ 2 วินาทีเพื่อให้เห็นข้อความ loading
    });
});
console.log('JavaScript Loaded'); // ตรวจสอบว่าคำสั่งนี้แสดงใน Console หรือไม่
