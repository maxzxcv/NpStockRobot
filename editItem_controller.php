...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $updated_values = [];
    $changes = []; // เก็บค่าที่เปลี่ยนแปลง
    $history_values = [];
    foreach ($_POST as $key => $value) {
        $old_value = isset($data[$key]) ? $data[$key] : 'N/A'; // ค่าเดิม
        $new_value = mysqli_real_escape_string($con, $value); // ค่าใหม่
        $updated_values[] = "$key = '$new_value'";

        // ตรวจสอบเฉพาะค่าที่เปลี่ยนแปลง
        if ($old_value != $new_value) {
            $changes[] = "$key: $old_value → $new_value";
            $history_values[$key] = "$old_value → $new_value";
        }
    }
    ...
    if (!empty($changes)) {
        ...
        if (mysqli_query($con, $update_query)) {
            // บันทึกลง Stock_Spare_history
            $date = date('Y-m-d H:i:s');
            $ItemNameHistory = isset($history_values['ItemName']) ? $history_values['ItemName'] : $data['ItemName'];
            $SerialNumberHistory = isset($history_values['SerialNumber']) ? $history_values['SerialNumber'] : $data['SerialNumber'];
            $StatusHistory = isset($history_values['status']) ? $history_values['status'] : $data['status'];
            $WhereItemHistory = isset($history_values['whereItem']) ? $history_values['whereItem'] : $data['whereItem'];

            $insertQuery = "INSERT INTO Stock_Spare_history (username, ItemName, SerialNumber, status, whereItem, Date) 
                            VALUES (
                                '" . mysqli_real_escape_string($con, $username) . "', 
                                '" . mysqli_real_escape_string($con, $ItemNameHistory) . "', 
                                '" . mysqli_real_escape_string($con, $SerialNumberHistory) . "', 
                                '" . mysqli_real_escape_string($con, $StatusHistory) . "', 
                                '" . mysqli_real_escape_string($con, $WhereItemHistory) . "', 
                                '$date'
                            )";
            mysqli_query($con, $insertQuery);

            // บันทึกการกระทำลง Log
            $action = "แก้ไขข้อมูลในตาราง";
            $details = "ผู้แก้ไข: $username, แก้ไขข้อมูล ID: $id, เปลี่ยนแปลง: " . implode(', ', $changes);
...
document.addEventListener('DOMContentLoaded', function() {
    Swal.fire({
        ...
        window.location.href = 'stock_controller.php';
    });
});
</script>";
        ...
        echo "<script>alert('ไม่มีการเปลี่ยนแปลงข้อมูล'); window.location.href = 'stock.php';</script>";
    }
}
$column_labels = [
    'id' => 'ไอดี',
...
<body>
    ...
    <select class="form-control" id="<?= $key ?>" name="<?= $key ?>">
        ...
        <option value="not_active" <?= ($value === 'not_active') ? 'selected' : '' ?>>เสีย ❌</option>
            ...
            <?php elseif (in_array($key, ['id', 'ItemName'])) : ?>
                <!-- Read-only Field -->
    ...
    <a href="stock_controller.php" class="btn btn-secondary">ยกเลิก</a>
    <a href="#" class="btn btn-danger" onclick="confirmDelete(<?= $id ?>, 'Stock_Main2_KPS')">ลบทิ้ง</a>
</form>
    </div>
</body>
<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
